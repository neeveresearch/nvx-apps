...
APPCONF=conf/profiles/$1/ems1.conf
shift
...
#FLIGHT_RECORDER_OPTS="-XX:+UnlockCommercialFeatures -XX:+FlightRecorder -XX:StartFlightRecording=name=MyRecording,settings=profile -XX:FlightRecorderOptions=defaultrecording=true,dumponexit=true,dumponexitpath=ems1-recording.jfr"

$JAVA_HOME/bin/java $FLIGHT_RECORDER_OPTS $GC_ARGS -Dnv.app.propfile=$APPCONF -cp "libs/*" com.neeve.server.Main -n ems1 $*
